...
public interface JobPostRepository extends JpaRepository<JobPost, Long> {
	...
	@Query("Select j from JobPost j where j.clinic.organization = 1 and j.status = 3")
	List<JobPost> findJobPostsWithOutstandingPayment();
	
	@Query("Select j from JobPost j where j.clinic.organization = 1 and j.status = 4")
	List<JobPost> findPaidJobPosts();

	@Query("Select j from JobPost j where j.clinic.organization = 1 and (j.status = 3 OR j.status = 4)")
